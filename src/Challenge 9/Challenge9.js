...
const ProjectileMotionComparison = () => {
    ...
    const chartRefs = {
        ...
        horizontalVelocity: useRef(null),
        verticalVelocity: useRef(null),
        totalVelocity: useRef(null),
    };
    ...
    const calculateTrajectoryPoints = useCallback(() => {
        ...
        let xNoRes = 0;
        let yNoRes = launchHeight;
        let vxNoRes = vx;
        let vyNoRes = vy;
        let t = 0;
        let dt = 0.01;
    
        const k = 0.5 * dragCoefficient * airDensity * crossSectionalArea / objectMass;
    
        while (y >= 0 && yNoRes >= 0) {
            const v = Math.sqrt(vx * vx + vy * vy);
            const vNoRes = Math.sqrt(vxNoRes * vxNoRes + vyNoRes * vyNoRes);
            const ax = -k * v * vx;
            ...
            xNoRes += vxNoRes * dt;
            yNoRes += vyNoRes * dt - 0.5 * gravity * dt * dt;
            vyNoRes -= gravity * dt;
        ...
        return data;
    }, [launchHeight, launchAngle, gravity, launchSpeed, dragCoefficient, crossSectionalArea, airDensity, objectMass]);
    ...
    const getChartData = (data, xKey, yKey, yKeyNoRes) => ({
        datasets: [
            {
            ...
            },
            {
                ...
                data: data.filter(point => point && point[xKey] !== undefined && point[yKeyNoRes] !== undefined)
                          .map(point => ({ x: point[xKey], y: point[yKeyNoRes] })),
            ...
            {
                label: 'Ball (With Air Resistance)',
            ...
            {
                label: 'Ball (Without Air Resistance)',
                data: data[animationIndex] && data[animationIndex][xKey] !== undefined && data[animationIndex][yKeyNoRes] !== undefined
                    ? [{ x: data[animationIndex][xKey], y: data[animationIndex][yKeyNoRes] }]
                    : [],
    ...
    return (
        <div className="trajectoryCalculator">
            <h1 className="title">Challenge 9</h1>
...
<div className="buttonContainer">
            ...
            <div className="chartContainer">
                <Line
                    data={getChartData(trajectoryData, 'x', 'y', 'yNoRes')}
            ...
            <div className="chartContainer">
                <Line
                    data={getChartData(trajectoryData, 't', 'y', 'yNoRes')}
            ...
            <div className="chartContainer">
                <Line
                    data={getChartData(trajectoryData, 't', 'vx', 'vxNoRes')}
                    options={getChartOptions('t', 'Velocity (m/s)', 'Horizontal Velocity vs Time')}
                    ref={chartRefs.horizontalVelocity}
                />
                <div className="buttonContainer">
                    <button onClick={() => saveAsPNG(chartRefs.horizontalVelocity, 'Horizontal Velocity')} className="btn btnPrimary saveButton">
                        Save as PNG
                    </button>
                </div>
            </div>
            <div className="chartContainer">
                <Line
                    data={getChartData(trajectoryData, 't', 'vy', 'vyNoRes')}
                    options={getChartOptions('t', 'Velocity (m/s)', 'Vertical Velocity vs Time')}
                    ref={chartRefs.verticalVelocity}
                />
                <div className="buttonContainer">
                    <button onClick={() => saveAsPNG(chartRefs.verticalVelocity, 'Vertical Velocity')} className="btn btnPrimary saveButton">
                        Save as PNG
                    </button>
                </div>
            </div>
            <div className="chartContainer">
                <Line
                    data={getChartData(trajectoryData, 't', 'v', 'vNoRes')}
                    options={getChartOptions('t', 'Velocity (m/s)', 'Total Velocity vs Time')}
                    ref={chartRefs.totalVelocity}
                />
                <div className="buttonContainer">
                    <button onClick={() => saveAsPNG(chartRefs.totalVelocity, 'Total Velocity')} className="btn btnPrimary saveButton">
                        Save as PNG
                    </button>
                </div>
            </div>
    ...
    <div className="table-wrapper">
        <table>
            <thead>
                <tr>
                    <th>Time (s)</th>
                    <th>X (m) with Air Resistance</th>
                    <th>Y (m) with Air Resistance</th>
                    <th>X (m) without Air Resistance</th>
                    <th>Y (m) without Air Resistance</th>
                </tr>
            </thead>
            <tbody>
                {tableData.map((point, index) => (
                    <tr key={index}>
                        ...
                        <td>{point.xNoRes.toFixed(2)}</td>
                        <td>{point.yNoRes.toFixed(2)}</td>
                    </tr>
                ))}
            </tbody>
        </table>
